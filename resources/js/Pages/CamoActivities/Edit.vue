...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2>Edit Activity <small>{{ props.resource.data.id }}</small></h2>
        </template>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <div class="flex flex-row space-x-3">
                            <div>
                                ...
                                <input id="name" v-model="form.name"
                                       class="rounded-md border-gray-100 bg-gray-200" disabled
                        ...
                        <div class="flex flex-row justify-items-center space-x-3">
                            <div class="my-1">
                                ...
                                <input id="labor_mount" v-model="form.labor_mount"
                                       ...
                                       name="labor_mount" step="0.01"
                                       type="number">
                            </div>
                            <div class="my-1">
                                <InputLabel for="material_mount">Material Mount</InputLabel>
                                <input id="labor_mount" v-model="form.material_mount"
                                       class="text-right rounded-md border-gray-300"
                                       name="labor_mount" step="0.01"
                                       type="number">
                            </div>
                        ...
                        <div class="flex justify-end space-x-4">
                            <PrimaryButton v-if="form.isDirty" type="submit">Save</PrimaryButton>
                            <SecondaryButton @click="goBack">Go Back</SecondaryButton>
                        </div>

                        <div v-if="Object.keys(form.errors).length > 0" class="border rounded-md px-7 py-2 my-2">
                            <ul class="list-decimal">
                                <li v-for="(item, idx) in form.errors" class="text-sm text-red-500">{{ item }}</li>
                            </ul>
                        </div>
                    </div>
                    <!-- col 2 -->
